...
# [Test loadingbar]
def downloadBar(url, formatting="{l_bar}{color}{bar}{reset}{r_bar}", chars=None, ncolsi=110, uniti='iB', block_size=1024,):
    ...
    print(f"Downloading {filename}... (OBS! Total size is an estimate)")
    site = urlopen(url)
    meta = site.info()
    # Streaming, so we can iterate over the response.
    response = requests.get(url, stream=True)
    total_size_in_bytes = int(meta["Content-Length"])
    if total_size_in_bytes == None or total_size_in_bytes == "" or total_size_in_bytes == int():

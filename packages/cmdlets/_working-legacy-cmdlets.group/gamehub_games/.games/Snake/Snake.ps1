...
# Load gamehub api
$script:apifile = "$PSScriptRoot\..\..\.gamehub_assets\gamehub_api.ps1"
. "$apifile"
...
#  Show scoreboard
  ...
  $gamehub_scores = gamehub_userscore -game "snake" -getscores
  [string]$gamehub_scoreraw = "$gamehub_scores"
  $gamehub_scoreraw = $gamehub_scoreraw.trim("@|}|{")
  $gamehub_scoreraw = $gamehub_scoreraw -replace " ",""
  [array]$gamehub_scoreboard = $gamehub_scoreraw -split ';'
  $gamehub_scoreboard = $gamehub_scoreboard | sort {calcSort($_)} -Descending
  [int]$gamehub_maxLength = 0
  write-host ""
  write-host "  Online scoreboard for Snake: (gamehub_sync)" -f green
  write-host "===============================================" -f darkgray
  foreach ($gamehub_s in $gamehub_scoreboard) {
    $gamehub_user = ($gamehub_s -split '=')[0]
    if ($gamehub_user.Length -gt $gamehub_maxLength) {
        $gamehub_maxLength = $gamehub_user.Length
    }
  }
  foreach ($gamehub_s in $gamehub_scoreboard) {
    [array]$gamehub_sA = $gamehub_s -split '='
    [string]$gamehub_user = $gamehub_sA[0]
    $gamehub_user += " "*([int]$gamehub_maxLength - $gamehub_user.Length)
    $gamehub_score = $gamehub_sA[1]
  ...
  while ($gamehub_loop) {
    ...
    if ($gamehub_username_q -eq "exit") {exit}
  ...
  if ("$debug" -eq "$true") {
    #gamehub_saveService_on -abs -dataFile "$script:gamehub_savefile" -dodebug
  } else {
    #gamehub_saveService_on -abs -dataFile "$script:gamehub_savefile"
  }
    ...
    Switch ($action) {
        'Collision' {
            if ($timer.Tag -ceq 'Collision') {
                # Gamehub additions
                [int]$gamehub_gamescore = $script:gamehub_gamelastscore
                $gamehub_tempout = "$script:gamehub_username : $gamehub_gamescore"
                if ($option_SafeSave -eq "True") {
                    if (test-path "$script:gamehub_savefile") {
                    $gamehub_retrivedlastsavedata = get-content "$script:gamehub_savefile"
                    $gamehub_unenc_retrivedlastsavedata = encdec "$gamehub_retrivedlastsavedata" -dec
                    [array]$gamehub_unenc_retrivedlastsavedataA = $gamehub_unenc_retrivedlastsavedata -split ' : '
                    [int]$gamehub_retrivedlastscore = $gamehub_unenc_retrivedlastsavedataA[0]
                    } else {
                    [int]$gamehub_retrivedlastscore = 0
                    }
                } else {
                    [int]$gamehub_retrivedlastscore = $gamehub_gamescore - 1
                }
                if ($gamehub_gamescore -gt $gamehub_retrivedlastscore) {
                  gamehub_filehand -file "$script:gamehub_savefile" -string "$gamehub_tempout" -abs -enc
                }
